'''
This code file is used to run the code programs for TeamLLM, LLM Discussion, MAD and Single-LLM.
'''

from Collaboration import RoleplayCollaboration, Single, LLMDiscussion
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from Process_Results.Write_Answers import complete_answer, write_single_answer
...
def run_Team(Model_Type, Model_Name, Model_ID, display = 0):
    for FS_str in FS_lst: # run through all future scenarios one by one
        ...
        print('\n', "============   " + name + ' (' + Model_Name + ')' + '   ============\n')
        Team = RoleplayCollaboration(Model_Type, Model_Name, Model_ID, display = display)
...
def run_LLM_Discussion(Model_Type, Model_Name, Model_ID):
    for FS_str in FS_lst: # run through all future scenarios one by one
        name = 'B' + Model_ID + '_' + FS_str
        print('\n', "============   " + name + ' (' + Model_Name + ')' + '   ============\n')
        Discussion = LLMDiscussion(Model_Type, Model_Name, Model_ID)

        for step in range(7):
            history = Discussion.run(step, FS_str)

        complete_answer(history, name, 'B' + Model_ID)

def run_MAD(Model_Type, Model_Name, Model_ID):
    for FS_str in FS_lst: # run through all future scenarios one by one
        name = 'C' + Model_ID + '_' + FS_str
        print('\n', "============   " + name + ' (' + Model_Name + ')' + '   ============\n')
        Team = RoleplayCollaboration(Model_Type, Model_Name, Model_ID)

        for step in range(7):
            history = Team.run(step, FS_str)

        complete_answer(history, name, 'C' + Model_ID)

    

def run_Single(Model_Type, Model_Name, Model_ID):
    for FS_str in FS_lst:
        name = 'D' + Model_ID + '_' + FS_str
        print('\n', "============   " + name + ' (' + Model_Name + ')' + '   ============\n')
        ...
        write_single_answer(step_answer, name, 'D' + Model_ID)
...
# for Model_Type, Model_Name, Model_ID in Model_lst:
#     run_LLM_Discussion(Model_Type, Model_Name, Model_ID)

# for Model_Type, Model_Name, Model_ID in Model_lst:
#     run_MAD(Model_Type, Model_Name, Model_ID)