'''
This code file is used to implement the running code for TeamLLM, LLM Discussion, MAD and Single-LLM.
'''
...
class GeneralExperiment():
    def __init__(self, Model_Type, Model_Name, Model_ID):
        self.FPSP_FS = self.load_json(r'Datasets\future_scenarios_en.json')
...
class RoleplayCollaboration(GeneralExperiment):
    def __init__(self, Model_Type, Model_Name, Model_ID, display):
...
class LLMDiscussion(GeneralExperiment):
    def __init__(self, Model_Type, Model_Name, Model_ID, display = 0):
        super().__init__(Model_Type, Model_Name, Model_ID)
        self.TeamRole = self.load_json(r'prompts\LLM_Discussion_Roles.json')
        self.role_play = [] # for storing role-play prompts for each agent
        self.NAME_LST = [
            "Environmentalist",
    ...
    def run(self, step, FS_str): # Here we implement the logic for step-by-step discussion.
        ...
        '''LLM Discussion Initial Prompting'''
        for agent in self.agents:
            agent.set_meta_prompt(self.construct_meta_prompt(FS_str))
        step_prompt = super().construct_step_prompt(step)
        self.agents[0].add_user(self.role_play[0] + '\n\n' + step_prompt)
        initial_responses = self.agents[0].ask()
        self.agents[0].add_assistant(initial_responses)
        step_history = self.update_step_history(self.agents[0], initial_responses, step_history)

        for i, agent in enumerate(self.agents[1:], start = 1):
            
            agent.add_user(self.role_play[i] + '\n\n' + 'These are the solutions to the problem from other agents: ' + '\n\n'.join(self.log_his + step_history) \
                           + '\n\n' + 'You are in a group discussion with other teammates; as a result, answer as diversely and creatively as you can.')
            ans = agent.ask()
            agent.add_assistant(ans)
            step_history = self.update_step_history(agent, ans, step_history)
        plain_answer = step_history[-1][step_history[-1].index('：') + 1:].strip()
...
class MAD(GeneralExperiment):
    def __init__(self, Model_Type, Model_Name, Model_ID, display = 0):
        super().__init__(Model_Type, Model_Name, Model_ID)
        self.TeamRole = self.load_json(r'prompts\MAD_Roles.json')
        self.role_play = [] # for storing role-play prompts for each agent
        self.NAME_LST = [
            "Moderator",
            "Debater A",
    ...
    def run(self, step, FS_str): # Here we implement the logic for step-by-step discussion.
        ...
        # Debater A
        self.agents[1].add_user(self.role_play[0] + '\n\n' + step_prompt)
        initial_responses = self.agents[1].ask()
        self.agents[1].add_assistant(initial_responses)
        step_history = self.update_step_history(self.agents[1], initial_responses, step_history)

        # Debater B
        self.agents[2].add_user(self.role_play[2] + '\n\n' + 'The following is the historical record up to now: ' + '\n\n'.join(self.log_his + step_history))
        initial_responses = self.agents[2].ask()
        self.agents[2].add_assistant(initial_responses)
        step_history = self.update_step_history(self.agents[2], initial_responses, step_history)
