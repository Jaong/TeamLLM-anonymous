...
**NOTE:** Please note that the data in this repository is not yet complete. During the double-blind review stage, only one scenario and data presented in the paper appendix are publicly available. We will release scenarios, full-process responses and the corresponding human evaluation data after acceptance.
...
TeamLLM is a multi-LLM collaboration framework inspired by **Belbin’s Team Role Theory**. Instead of assigning LLMs to distinct functions or random roles, TeamLLM organizes LLMs into **human-like team roles**, enabling creative idea generation and comprehensive problem-solving.
...
1. **Task Initiation**  
   - Co-Ordinator introduces the current step, clarifies requirements, and guides discussion.

2. **Perspective Sharing**  
   - LLMs contribute sequentially according to their roles:
     - Plant proposes initial ideas.
     - Monitor Evaluator critiques and refines.
     - Implementer assesses feasibility.

3. **Consensus Building**  
   - Co-Ordinator integrates all viewpoints into a coherent, step-specific final answer.
...
To properly evaluate TeamLLM, we introduce **CGPST (Contextually-Grounded and Procedurally-Structured Tasks)** — a benchmark specifically designed for **Multi-Step Contextualized Tasks**. Unlike traditional single-step QA benchmarks, CGPST is characterized by the following four key properties.
...
Instead of scoring only the final answer, CGPST evaluates **step-wise outputs**. This enables fine-grained evaluation of LLMs.

#### 4️⃣ Multi-Dimensional Assessment  
Each step is scored across multiple dimensions (e.g., originality, relevance), capturing diverse abilities such as Divergent thinking, Convergent thinking, Problem-Solving.
...
1. Full implementation of TeamLLM framework and all baselines: *Experiments/experiment.py*.
...
7. Complete experiment results with xlsx format: *Results*
8. Data processing: *Process_Results*
9. Automatic Scoring using LLM-as-a-Judge: *Automatic Scoring*
